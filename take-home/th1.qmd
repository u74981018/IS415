...
-   [ANN MEI YI VICTORIA GRACE](https://is415-gaa-vic.netlify.app/take-home_ex/take-home_ex01/take-home_ex01). ![](https://is415-ay2024-25t1.netlify.app/img/thumbup1.jpg) The discussion on [Network-Constrained Kernel Density Estimation (NKDE) & Analysis](https://is415-gaa-vic.netlify.app/take-home_ex/take-home_ex01/take-home_ex01a#network-constrained-kernel-density-estimation-nkde-analysis) section is very interesting.

-   [KHANT MIN NAING](https://is415-geospatial-with-khant.netlify.app/take-home_ex/take-home_ex01) ![](https://is415-ay2024-25t1.netlify.app/img/thumbup1.jpg). A very well researched submission. Did exceptionally well in performing geospatial analysis and geocommucating and did well in geospatial data wrangling and geovisualisation.

-   [MATTHEW HO YIWEN](https://is415-gaa-matthew-ho.netlify.app/takehomeex/takehomeex1/the1)![](https://is415-ay2024-25t1.netlify.app/img/thumbup1.jpg) Did exceptionally well in geospatial data wrangling and reproducibility, excellence in geospatial analysis and moderate well in geovisualisation and geocommunication.

# My Task

## Using appropriate function of **sf** and **tidyverse** packages, import and transform the downloaded armed conflict data and administrative boundary data into sf tibble data.frames.

Import sf, tmap and tidyverse
...
```{r}
# Read the CSV as a regular data frame
data <- read.csv("data/1data/2021-01-01-2024-06-30-Myanmar.csv")

# Convert to a simple feature if you have longitude and latitude columns
data_sf <- st_as_sf(data, coords = c("longitude", "latitude"), crs = 4326)
```

```{r}
mmr = st_read("data/1data/geospatial/geonode-mmr_polbnda2_adm1_250k_mimu_1.kml")
```

```{r}
# https://geonode.themimu.info/layers/geonode%3Ammr_polbnda_adm1_250k_mimu_1

mmr2 = st_read("data/1data/geospatial/geonode-mmr_polbnda_adm1_250k_mimu_1.kml")
```
...
```{r}
tmap_mode('plot')
mmr2 <- st_transform(mmr2, crs = 4326)
```

```{r}
# Check if all geometries are valid
invalid_geometries <- st_is_valid(mmr2)
summary(invalid_geometries)

# If some are invalid, fix them
mmr2_valid <- st_make_valid(mmr2)

# Plot the valid geometries with tmap
tm_shape(mmr2_valid) +
...
{r} plot(st_geometry(mmr), col = 'lightblue', border = 'black', main = "Myanmar Regions1")+ plot(st_geometry(mmr2), col = 'lightblue', border = 'black', main = "Myanmar Regions2")+ tm_shape(data_sf)+ tm_dots()
